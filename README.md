# Awesome AI Testing [![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

> A curated list of tools, frameworks, and resources for testing AI and machine learning systems.

[![License: CC0-1.0](https://img.shields.io/badge/License-CC0_1.0-lightgrey.svg)](https://creativecommons.org/publicdomain/zero/1.0/)

## Contents

- [LLM Evaluation Frameworks](#llm-evaluation-frameworks)
- [Articles & Tutorials](#articles--tutorials)
- [Contributing](#contributing)

## LLM Evaluation Frameworks

- [DeepEval](https://github.com/confident-ai/deepeval) - Open-source LLM evaluation framework with 14+ metrics including hallucination detection.
- [Promptfoo](https://github.com/promptfoo/promptfoo) - Test and evaluate LLM prompts with systematic comparison and CI/CD integration.
- [Inspect AI](https://github.com/UKGovernmentBEIS/inspect_ai) - UK AI Safety Institute framework for evaluating LLM capabilities and safety.
- [Ragas](https://github.com/explodinggradients/ragas) - Evaluation framework for Retrieval Augmented Generation (RAG) pipelines.
- [Giskard](https://github.com/Giskard-AI/giskard) - Testing framework for ML models covering bias, performance, and security.
- [Langsmith](https://www.langchain.com/langsmith) - Platform for debugging, testing, evaluating, and monitoring LLM applications.
- [Arize Phoenix](https://github.com/Arize-ai/phoenix) - Open-source ML observability for LLMs with tracing and evaluation.
- [TruLens](https://github.com/truera/trulens) - Evaluation and tracking for LLM applications with feedback functions.
- [OpenAI Evals](https://github.com/openai/evals) - Framework for evaluating LLMs and AI systems from OpenAI.

## Articles & Tutorials

- [A Survey on Evaluation of Large Language Models](https://arxiv.org/abs/2307.03109) - Academic survey of LLM evaluation methods.

---

## Contributing

Contributions welcome! Please read the [contribution guidelines](CONTRIBUTING.md) first.

## License

[![CC0](https://licensebuttons.net/p/zero/1.0/88x31.png)](https://creativecommons.org/publicdomain/zero/1.0/)

To the extent possible under law, the contributors have waived all copyright and related or neighboring rights to this work.
